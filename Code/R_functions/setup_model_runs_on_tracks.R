setup_model_runs_on_tracks <- function(df,N,tau,model,initial_depth,integration_factor,f_time){
...
for (i in 1:length(splitdf)){
  # for (i in 41){
  sdf= splitdf[[i]]
  start_frame = max(splitdf[[i]]$change_frame[1]- splitdf[[i]]$frame[1],8)
  end_frame = nrow(sdf)
  flowVel = c(0,0,0)

  setup <-setup_track_flow(df=sdf, fps=100,integration_factor,start_frame,end_frame,initial_depth,tau)
 ...
 # h_out = rbind(h_out, simOut[[4]])
  BA_out = c(BA_out, simOut[[6]])
  dBA_out =  c(dBA_out, simOut[[6]])
}
mod_out<-data.frame(ID=ID_out, frame= frame_out,rx_m = r_m_out[,1], ry_m = r_m_out[,2],rz_m = r_m_out[,3],vx=v_m_out[,1],vy=v_m_out[,2],dBA_out=dBA_out,BA_out=BA_out)
...
splitData<-split(df_out, df_out$ID)
